...
**Verification.** Cross-package import chain (Thrash Margin's `_lib` + Niccolo's own sim engine + the `@repo/engine` workspace package + JSON content) confirmed to bundle and execute cleanly via a smoke test that invoked the built API handlers directly with a mocked request/response and an unreachable database — request pipelines ran end-to-end up to the expected connection failure, with no import or reference errors. Live in the browser: a fresh Lobby correctly showed no campaigns, "Begin →" created "Campaign #1" and navigated to it, the week-0 event fired and resolved normally, "← Back to campaigns" returned to the Lobby showing the save with its correct week and status, and a full page reload preserved it. Cloud sign-in/save could not be exercised live — no Supabase credentials were available in this session — so the cloud path is verified only by the import/execution smoke test above, not a live round trip; worth a manual check after the schema migration (see point 1) is run against production. `tsc --noEmit` and `vite build` both clean throughout.

---

## Decisions needed from you

**Action required:** run the `game` column migration against the live Supabase database before cloud saves will work in production — `npm run migrate` from `packages/thrash-margin/server/` with `DATABASE_URL` set to the production connection string (or run `packages/thrash-margin/server/src/db/schema.sql` directly via the Supabase SQL editor). No database credentials were available in this session to run it directly. Until it's run, `/api/niccolo/game*` will fail with a "column does not exist" error; Niccolo's local (non-signed-in) save/load is unaffected and already works.
...
- Verified the API import chain (cross-package `_lib` + Niccolo's sim engine + `@repo/engine` + JSON content) bundles and executes cleanly via esbuild plus a smoke test against a mocked request and an intentionally-unreachable database. Verified the full local flow live in the browser: Lobby → begin campaign → play → back to Lobby → reload, all correct. Cloud sign-in/save could not be exercised live (no Supabase credentials in this session) — flagged as needing a manual check once the schema migration is run.
- `tsc --noEmit` and `vite build` both clean.
