...
import type { Haulier, LogEntry, LogKind } from '../sim/types';

/**
 * Pops a card for anything notable, so the game can be watched rather than read.
 *
 * An early report was "theft doesn't seem to do anything". It does — measured over 1,394 rounds,
 * 268 delays, 70 recoveries and 8 seizures, roughly a delay every five rounds. But every one of them
 * produced only a line in a scrolling log, and if it fired during a computer haulier's turn nobody
 ...
 * cast-off, and promoting those to cards buried the delay that had just happened under three
...
const PRESENTATION: Partial<Record<LogKind, { title: string; colour: string; glyph: string }>> = {
  delay: { title: 'Heavy weather', colour: UI.warn, glyph: '🌊' },
  theft: { title: 'Theft', colour: UI.bad, glyph: '⚔' },
  ...
  return (
    <div style={stack} aria-live="polite">
      {queue.map(entry => {
        ...
        const haulier = hauliers.find(c => c.id === entry.haulierId);
        const mine = entry.haulierId !== null && entry.haulierId === viewerId;
