import { sourcesFor, GOOD_BY_ID, distanceBetween, depotName } from '../sim/content';
...
import type { Haulier, Contract, Vehicle } from '../sim/types';

interface Props {
  contracts: Contract[];
  hauliers: Haulier[];
  /** The vehicle whose position the "how far" figures are measured from, if one is selected. */
...
/**
 * The five face-up commissions. This is the screen's centre of gravity — it is what every haulier
...
export default function ContractBoard({
  ...
  const nameOf = (id: string) => hauliers.find(c => c.id === id)?.name ?? 'someone';
    ...
    <Panel title="The exchange" aside={<Label>five commissions</Label>}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {contracts.map(contract => {
          ...
          // Every depot that stocks the good, nearest the destination first — the list a haulier
          // actually reads when deciding where to load.
          const sellers = sourcesFor(contract.good, contract.destination);

          // Distance is measured from wherever the selected vehicle actually is: if she is empty the
          // run starts at whichever depot selling this good is nearest her, if she is already loaded
          // with the right cargo it starts at the destination. Anything else would be answering a
          // question nobody asked.
          //
          // "Nearest seller" rather than a named source depot, because the card names no source —
          // sourcing is the haulier's decision, and this line is the number that decision needs.
          let reach: string | null = null;
          let nearestSeller: string | null = null;
          if (reference) {
            const at = reference.location ?? reference.run?.route[reference.run.route.length - 1];
            if (at) {
            ...
            >
              ...
              <span style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', minWidth: 0 }}>
                ...
                <span style={{ ...bodySmall, color: UI.textSoft }}>
                  wanted at <strong style={{ color: UI.text }}>{depotName(contract.destination)}</strong>
                ...
                {/* Where it can be had. The card does not dictate a source, so the useful thing to
                    show is the choice: nearest first, with the selected vehicle's nearest picked out. */}
                <span style={{ ...bodySmall, color: UI.textFaint, fontSize: '0.72rem' }}>
                  load at{' '}
                  {sellers.length === 0
                    ? 'nowhere on this chart'
                    : sellers.map((depotId, i) => (
                        <span key={depotId}>
                          {i > 0 && ', '}
                          <span
                            style={
                              depotId === nearestSeller
