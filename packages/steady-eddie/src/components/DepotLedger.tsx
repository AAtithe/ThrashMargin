...
import { sourcesFor, GOOD_BY_ID, DEPOTS, distanceBetween } from '../sim/content';
import { FONT, UI, money } from '../theme';
import { Panel } from './ui';
import type { Contract, GoodId, DepotId, Vehicle } from '../sim/types';

interface Props {
  contracts: Contract[];
  /** Distances are measured from this vehicle, when one is selected. */
  reference: Vehicle | null;
  /** Clicking a depot here does the same thing as clicking it on the chart. */
...
/**
 * What every depot trades, on the board at all times — the printed table the 1988 board carried
 * around the edge of its map.
 *
 * This is reference, not decoration. Without it, working out where to load a cargo means clicking
 * depots one at a time and reading the Orders panel, which is exactly the kind of hunting a printed
 * board never made you do. Sells and buys are separate views rather than crammed together: at any
 * given moment a haulier is asking one question or the other, never both.
 */
export default function DepotLedger({ contracts, reference, onDepotClick, targetDepot }: Props) {
  const [mode, setMode] = useState<Mode>('sells');
  const [onlyWanted, setOnlyWanted] = useState(false);

  /** Goods a live commission wants loaded at, or landed at, each depot. */
  const { hotSources, hotSinks } = useMemo(() => {
    const sources: Record<DepotId, Set<GoodId>> = {};
    const sinks: Record<DepotId, Set<GoodId>> = {};
    for (const c of contracts) {
      if (c.fills.length >= 2) continue;
      // Any depot stocking the good is a place a live commission can be loaded, not one named depot.
  ...
  const from = reference?.location ?? reference?.run?.route[reference.run.route.length - 1] ?? null;

  const grouped = useMemo(() => {
    const hot = mode === 'sells' ? hotSources : hotSinks;
    return REGION_ORDER.map(region => ({
      region,
      depots: DEPOTS.filter(p => p.region === region).filter(
  ...
  return (
    <Panel
      title="What the depots trade"
      aside={
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <div style={switcher} role="group" aria-label="Show what depots sell or buy">
    ...
    >
        ...
        {grouped.map(group => (
          <div key={group.region} style={{ display: 'contents' }}>
            ...
            {group.depots.map(depot => {
              const goods = mode === 'sells' ? depot.supplies : depot.demands;
              const hot = (mode === 'sells' ? hotSources : hotSinks)[depot.id];
              const away = from && from !== depot.id ? distanceBetween(from, depot.id) : null;
              const isTarget = depot.id === targetDepot;
                          ...
                          <span
                            ...
                            title={
                              wanted
                                ...
                                : mode === 'sells'
                                  ? `${good?.name} — ${depot.name} asks ${money(
                                      priceAt(depot.id, id),
      ...
      <p style={footnote}>
        {mode === 'sells'
          ? 'Where a cargo can be loaded. A filled chip means a face-up commission wants that good from this depot.'
          : 'Where a cargo can be landed or, failing a commission, dumped at half price.'}
        {from && ' Distances are from the selected vehicle.'}
      </p>
    </Panel>
  );
}
...
const depotCell: React.CSSProperties = {
  display: 'flex',
...
const depotHead: React.CSSProperties = {
  display: 'flex',
  alignItems: 'baseline',
