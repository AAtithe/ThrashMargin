 ...
 * depot or good that no longer exists is skipped at draw time rather than crashing.
 *
...
import { GOODS, GOOD_BY_ID, DEPOTS, DEPOT_BY_ID, distanceBetween } from './content';
import { shuffle } from './rng';
import type { Contract, ContractFill, GoodId, DepotId } from './types';

export type CardKey = string;

const key = (good: GoodId, destination: DepotId): CardKey => `${good}|${destination}`;

export function parseCardKey(k: CardKey): { good: GoodId; destination: DepotId } | null {
  const parts = k.split('|');
  // Two parts is the current form. Three is a save from when cards named a source depot: the middle
  ...
  if (!GOOD_BY_ID[good] || !DEPOT_BY_ID[destination]) return null;
  return { good, destination };
}

/**
 * Every legal card: a good and a depot that wants it, with at least one depot selling that good near
 ...
 * supplier has to be within CONTRACT_MAX_DISTANCE of the buyer, but which one the haulier uses is
...
export function buildDeck(): CardKey[] {
  ...
  for (const good of GOODS) {
    const sources = DEPOTS.filter(p => p.supplies.includes(good.id));
    for (const d of DEPOTS.filter(p => p.demands.includes(good.id))) {
...
/** What the next delivery on this contract would pay. Zero once two hauliers have filled it. */
