 ...
 * other three games use (see packages/tea-race/api/game/*, discriminated by `game = 'steady_eddie'`).
...
export function useGameCloud() {
  ...
  const createGame = useCallback(
    async (name?: string, opts?: NewGameOptions): Promise<string | null> => {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) {
          setError(data.message ?? 'Failed to start the run');
          return null;
        }
        setState(data.state);
        await fetchSaves();
        ...
        setError('Network error — failed to start the run');
        return null;
      }
    },
    [fetchSaves],
  ...
  const loadGame = useCallback(async (gameId: string) => {
    setError(null);
    try {
      const res = await fetch(`${API}/api/steady-eddie/game/${gameId}`, { headers: authHeaders() });
      if (res.status === 401) {
        setError(SESSION_EXPIRED);
        return;
      }
      const data = await res.json();
      if (!res.ok) {
        setError(data.message ?? 'Failed to load the run');
        return;
      }
      setState(migrateState(data.state));
    } catch {
      setError('Network error — failed to load the run');
    }
  }, []);
  ...
  const dispatch = useCallback(
    (action: GameAction) => {
      setState(prev => {
        ...
        fetch(`${API}/api/steady-eddie/game/${next.id}`, {
          method: 'PUT',
          headers: authHeaders(),
  ...
  const deleteGame = useCallback(async (gameId: string) => {
    try {
      await fetch(`${API}/api/steady-eddie/game/${gameId}`, {
        method: 'DELETE',
        headers: authHeaders(),
      });
