...
const INDEX_KEY = 'steadyeddie_saves';
const stateKey = (id: string) => `steadyeddie_save_${id}`;

export interface SaveMeta {
  id: string;
  name: string;
  /** Rounds elapsed. Called `turn` to match the shape the other three games' lobbies use. */
  turn: number;
...
function isCurrentShape(parsed: unknown): parsed is GameState {
  ...
  if (s.rules !== 'standard') return false;
  if (typeof s.rngSeed !== 'number' || typeof s.round !== 'number') return false;
  if (!Array.isArray(s.hauliers) || !Array.isArray(s.vehicles)) return false;
  ...
  if (!s.miles || !s.dice || !Array.isArray(s.log)) return false;
  // Every vehicle must have a hold — migrateState converts the old single-cargo shape first.
  if (!s.vehicles.every(sh => Array.isArray((sh as { hold?: unknown }).hold))) return false;
  if (typeof s.nextContractSeq !== 'number' || typeof s.nextVehicleSeq !== 'number') return false;
...
export function useGameLocal() {
  ...
  const createGame = useCallback((name?: string, opts?: NewGameOptions): string => {
    ...
    const gameName = name?.trim() || `Run #${idx.length + 1}`;
