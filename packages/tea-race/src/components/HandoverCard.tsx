...
import type { Captain, GameState } from '../sim/types';

/**
 * Hotseat pause between two human captains.
 *
...
export default function HandoverCard({
  ...
  const ships = state.ships.filter(s => s.ownerId === next.id);
  const atSea = ships.filter(s => !s.location).length;
