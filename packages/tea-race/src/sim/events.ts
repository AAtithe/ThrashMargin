/**
 * World events — the deck of things that happen *to* the world rather than to one ship.
 *
 * Storms and pirates already act on a single vessel on a single leg. This module is the other half:
 * news that changes what every captain should be doing this round. A strike shuts a port; a glut
 ...
 *    game into a state it cannot leave. That is what makes it safe to close a port at all.
 *  - **Nothing is drawn once the endgame has begun.** A declaration is a countdown with a fixed
 *    cost; a strike landing on the declarer's home port mid-count would decide the game by dice
...
import { GOODS, PORTS, goodName, portName } from './content';
import { nextInt, next } from './rng';
import type { GameState, GoodId, PortId, WorldEvent, WorldEventKind } from './types';
...
export const EVENT_DURATION: Record<WorldEventKind, [number, number]> = {
  // The two that *stop* trade are kept short on purpose: they cost every captain turns, and turns
...
/** Ports worth striking: somewhere that actually trades, so the news bites. */
const STRIKEABLE: PortId[] = PORTS.filter(p => p.supplies.length > 0 || p.demands.length > 0).map(p => p.id);

function headlineFor(kind: WorldEventKind, port: PortId | undefined, good: GoodId | undefined): {
  headline: string;
  detail: string;
} {
  switch (kind) {
    case 'strike':
      return {
        headline: `Strike at ${portName(port!)}`,
        detail: `The lumpers have downed tools at ${portName(port!)}. Nothing loads and nothing lands there until the dispute is settled.`,
      };
    case 'embargo':
      return {
        headline: `${goodName(good!)} embargoed`,
        detail: `An order in council forbids the lading of ${goodName(good!).toLowerCase()} in any port. Cargo already in the hold may still be landed.`,
      };
    case 'glut':
      return {
...
export function drawEvent(
  ...
  const { headline, detail } = headlineFor(kind, port, good);
...
/** Is this port shut to all trade — no lading, no landing? */
export const portStruck = (state: GameState, port: PortId): boolean =>
  activeOf(state).some(e => e.kind === 'strike' && e.port === port);
