...
> **Phase 20 implementation, confirmed 2026-08-09:** "escort" — the last word of that line, and unbuilt since the first draft — is now real, as part of Chapter 6's convoys (`sim/convoy.ts`). A convoy is several cargo-capable vessels formed up in one port that thereafter sail as one: dispatching any member sends them all, on the same route with the same queued plan, so an auto-continued journey can't split them at an intermediate stop. A hired escort costs 60f plus 4f a week and multiplies each member's weekly storm/piracy chance by 0.4 — §4's own "escorts lower it", not immunity — and lapses if its wages can't be met, exactly as the household's own do. Still unbuilt from this line: vessel *classes* (round ship vs. galley vs. caravel) and the speed differences between them, which belong with buying and selling ships (`freeplay-and-trading-design.md`, Part 4) rather than with convoying the hulls the player already has.
...
> **Phase 19 implementation, confirmed 2026-07-30:** `sim/divining.ts` and a `USE_DIVINING` action, unlocked by a Chapter 5 event flag. Three scope decisions worth stating outright. **Each of the three uses above is tied to one city** — water at St Catherine's, ore at Edinburgh, a direction at Cairo — requiring a docked vessel there, so the gift answers a question about ground the player is standing on rather than being a free flag collected from Bruges on turn one. **"Health cost" is a four-week recovery cooldown plus a hard three-use campaign cap, not a second health meter**: Nicholas isn't a `Character` record (the player *is* him), so there's no health stat to spend, and Conscience is the only personal meter the sim has; a parallel 0-100 bar for one chapter's ability would be a larger mechanic than this line asks for and would immediately raise how it recovers, what else damages it, and why the Ledger doesn't show it. Time is a cost this game already prices in weeks. **And divining never gates a chapter's spine** — every use opens an *additional* branch and no main thread requires one, which is the Chapter 4 soft-lock lesson applied in advance to a limited-use ability a player could exhaust or simply never discover.
>
> **The parentage dossier** (track 2 above) is the Evidence Board's long track, and Chapter 5 contributes three pieces to it without answering anything — see §11's own note on screen 7 for the model. The `rep.*` example in this section's own JSON has been wired since Phase 8.
...
Chapter 0: Claes begins. A short prologue before Niccolo Rising proper — added retroactively (Phase 12) once it became clear Chapter 1's own "0 capital and 3 friendships" line, below, had never actually been built. Claes starts with nothing (no ship, 0 capital, only the dispatch rider and 3 of the 6 officers) and earns his way into Chapter 1's existing start through a handful of scripted beats: a courier errand that teaches the map, a dye-job commission, two officers joining narratively. Ends with Marian formally naming him her factor — the moment that hands over the ship, a 40f stake, and the rest of the household, becoming Chapter 1's week 0 exactly as it already existed. Optional: a "Skip the prologue" choice at campaign creation drops straight into that same state directly.

> **Phase 12 implementation, confirmed 2026-07-23:** ships as 8 events in `packages/niccolo/src/content/events/chapter0.json`. Reuses Chapter 1's own map/characters entirely — no new cities, routes, or currencies. A new `EventEffects.grantVessel` (mirroring `joinCharacter`'s own idempotency) lets the finale hand over the cargo ship mid-campaign; `createInitialState`'s new `skipPrologue` option is what today's "start with everything" behaviour became, now opt-in rather than the only path. Full implementation notes recorded in `PROGRESS.md`'s own Phase 12 section, including two bugs a scripted driver and live-browser verification caught: an early version wrongly force-activated Chapter 2's own mid-campaign joiner (Diniz) on the skip-prologue path, and the companion guided-tour work (see Section 11's own note) initially targeted a courier-investment city that can never actually be invested in.
...
> **Phase 10 implementation, confirmed 2026-07-22:** ships as 25 events in the new `packages/niccolo/src/content/events/chapter3.json` — at the low end of §9's "25 to 40" range, matching Chapter 2's own precedent of staying lean when a chapter is one location and one arc. Cyprus ships as two new cities (Famagusta, the port and siege backdrop; Kouklia, the estate, with no market of its own — Naples's Phase 7 precedent for a delivery-point city with nothing to trade) and its own currency, `bezant` (`CurrencyId`'s seventh member, pegged low to read as a kingdom already living on borrowed time). Reached by one new sea route from Venice (5 weeks) plus a short Famagusta-Kouklia land hop (2 weeks) — the same generic vessel abstraction already lets the one "ship" kind travel both, as Chapter 1's cannon shipment already relied on. No new House: Cyprus's antagonist is a historical siege, not a rival trading company, so (unlike Trebizond's Doria) this phase adds no house content at all, the same scope discipline every phase has used against inventing content a chapter doesn't call for.
>
> **The production asset** (design doc §12's own line for this phase) is `sim/estates.ts`: `Estate { cityId, goodId, stage, weeksInStage, stockpile }`, singular and Kouklia-only on this first outing, the same reduced-fidelity discipline Phase 7's condotta and Phase 8's houses used for theirs. `plant` is folded into `ESTABLISH_ESTATE` (25f, gated on a `kouklia_estate_available` flag an event sets); `growing` (8 weeks) and `refining` (3 weeks, yielding a 15-unit batch) both advance automatically every `ADVANCE_WEEK` via `resolveWeeklyEstate`; `ready` is the one stage that waits on a deliberate player action, `HARVEST_ESTATE`, so harvest reads as a choice rather than another silent tick; `ship` is `SHIP_ESTATE_GOODS`, loading the stockpile onto a docked vessel exactly like any cargo bought at a market, so refined sugar sells through the existing trade system (Famagusta's own market) rather than a second parallel one.
>
> **Choose or be forced** (design pillar 4, "history is the deadline") is built literally: `ev_c3_004` lets the player back Zacco, back Carlotta, or name neither; naming neither is the only path into `ev_c3_005`'s forced allegiance (Zacco, historically the victor) once a floor date passes. Backing Carlotta doesn't get overridden — an explicit choice is honoured — but `ev_c3_008` still charges the cost of having backed the losing side on the historical schedule, regardless of what the player believed when they chose it, the same lesson Trebizond's own extraction taught two chapters earlier.
>
> **Katelina's scripted end** follows Felix's Chapter 1 precedent exactly: narrative-only, not a `Character` record, so no new engine hook was needed (`characterDeparts` stays reserved for actual roster departures, as it already was for Diniz). Her loss, the siege's outcome, and the Cyprus branch together gate `chapter3_complete`, which lifts and replaces `chapter2_complete`'s hard freeze the same way Phase 9 moved it off `chapter1_complete` — each chapter's completion flag is a mid-campaign marker for the next one to trigger on until the true end of shipped content moves again.
>
> **Verification.** A scripted driver (seeding a fresh campaign past `chapter2_complete` directly, the same isolation-testing precedent every prior phase used) exercised three branches against `processAction`/`resolveEvent`: the success path (back a faction, establish and harvest the estate, ship sugar to Venice ahead of the siege) reached `chapter3_complete` with `famagusta_siege_survived` set; the failure path (back Carlotta, let the clock run past both the 1464 political deadline and the 1466 siege deadline with nothing shipped) reached `chapter3_complete` with `carlotta_cause_failed`, `famagusta_fallen`, and `katelina_lost` all set, `famagusta_siege_survived` absent; the neutral path confirmed `chose_neutral_cyprus` holds off any allegiance until the forced-hand deadline, then flips to `sided_zacco`/`zacco_forced`. Live in the browser: a fresh campaign's week-0 event still resolves normally with the estate panel correctly absent until offered; injecting `kouklia_estate_available` into a live save exercised `EstatePanel` end-to-end (establish → growing display → stockpile), and the widened Chapter 2 map already had room for Famagusta and Kouklia without a further `viewBox` change. `tsc --noEmit` and `vite build` both clean.
...
> **Phase 19 implementation, confirmed 2026-07-30:** ships as 33 events in `packages/niccolo/src/content/events/chapter5.json` across 5 threads (Scotland/the Boyds, the hunt for the child, the Egyptian licence, the St Catherine's consignment, the Vatachino), plus 3 divining-bonus events — inside this section's own 25-40 range. Four new cities (`chapter5.json`): Edinburgh, Alexandria, Cairo, and St Catherine's (no market at all — the Naples/Kouklia precedent for a delivery point). Two new currencies close out §5's own list: `scots_pound` (deliberately *not* the existing English `pound`, pegged low for James III's debasement) and `dinar` (the strongest coin on the map, which is why Alexandria could dictate spice terms). Two new characters join mid-arc via the existing `joinCharacter` — Katelijne Sersanders at Edinburgh and John le Grant at Cairo. Two secrets, per this section's own "2 to 3": the Scottish chamberlain's forfeiture list, and next season's Mamluk spice rate.
>
> **The first chapter pack with no new goods file.** Scottish wool, Egyptian spices and sugar, and a Mamluk court market at Cairo paying the game's highest prices for Flemish cloth and Venetian glass are all existing goods in new markets — historically what those cities actually traded, and the only way to add four cities without opening four new dead ends (a Dijkstra sweep puts every good sold at a new city within 6 weeks of another market for it).
>
> **The set piece** is the St Catherine's consignment: buy Venetian glass at Venice and have the ship at the monastery still carrying at least 3 crates, then bring it back down to Cairo to close the account. Built deliberately to avoid the fragility of Chapter 4's own gold requirement — St Catherine's has no market, so there is nothing to sell the glass to early, and the last leg is land rather than a seasonal sea passage.
>
> **The hunt for the child does not end with the child.** Both branches set `child_withheld`; divining a direction at Cairo buys the boy's name and nothing else. That is pillar 3 and the novels' own answer, not an unfinished thread.
>
> **`EventTrigger.weeksAfterFlag`, a relative deadline, retires a compromise Chapters 2, 3 and 4 each recorded separately.** The clock runs continuously with no per-chapter reset, so an absolute `dateAfter` deadline is either already past when its chapter unlocks or absurdly far off — Chapter 4's own 1471 fallback is week 563. All three of Chapter 5's deadlines are relative to the week their own flag was set (a new optional `GameState.flagWeeks`), and the whole all-failure path closes in 72 weeks rather than never being reached at all.
...
> **Phase 20 implementation, confirmed 2026-08-09:** ships as 30 events in `content/events/chapter6.json` across the five threads this line names. Two new cities — Reykjavik and Danzig (the latter already in §3's list, and the seat of the new hostile Hanse house) — plus two goods from §4's own unimplemented list, `stockfish` and `furs`. One new character, Crackbene.
>
> **"Mass logistics" (§12's system line for this chapter) is convoys, not shipyards.** Several vessels sail as one, share a hired escort, and satisfy a combined-hold delivery check no single hull could. Buying and selling ships, vessel classes and per-hull upkeep are a separate, separately-owned spec (`freeplay-and-trading-design.md`, Part 4) sequenced after warehousing; Chapter 6 grants its extra hulls through the existing `grantVessel` effect instead, so convoys layer on top of fleet growth whenever that lands rather than duplicating it. The escort is §4's own last unbuilt word — see that section's note.
>
> **Godscalc's death is the first scripted death of a roster `Character`.** Felix's and Katelina's were narrative-only because neither was a `Character` record; Godscalc is, so §7's "the game does not allow the player to save them" is enforced here for the first time through `characterDeparts` on a founding officer. Both choices kill him; only whether his last letter is kept differs, and keeping it adds a piece to Chapter 5's parentage dossier.
>
> **Historical anchors were checked, not guessed** (§13): the Anglo-Hanseatic War ran 1469-1474 (Treaty of Utrecht, 28 February 1474) and is why the fishery is contested; alum was found on Volterra's own commune land in 1470 and Florence sacked the town on 30 June-1 July 1472, which is the bank's scripted catastrophe and ties back to Chapter 1's alum secret; and Frederick III left Trier before dawn on 24 November 1473 without crowning Charles, which is the Burgundy trap closing. All three are carried in prose and gated on relative `weeksAfterFlag` deadlines rather than the literal dates, since the campaign clock is continuous.
...
>
> **Phase 19 implementation, confirmed 2026-07-30:** the Vatachino ship as a fifth house (hostile, seat Alexandria), and this section's own parenthetical for them — "hidden ownership" — is built as two content fields plus one engine pass, generic over houses rather than special-cased to the Vatachino by name: `House.hiddenBackers { track, revealedByFlag, text }` and `House.backerLeads[]`, with `sim/dossier.ts`'s `resolveUnmasking` setting the flag once the player holds 3 items on that house's Evidence Board track. §9's "the intelligence system can unmask early or late" is built as two genuinely different routes: the chapter's own events hand over two leads, and the third comes either from a paid Genoese notary late in the chapter or from an agent placed inside the company (a new `resolveWeeklyAgentEvidence` at 8%/week — deliberately below the existing insider-secret roll's 15%, since who owns a company is what a masked house guards hardest). Neither route is required to resolve the Vatachino thread; they only decide whether it resolves with names attached. Being a fifth house, the Vatachino inherit the existing weekly trade footprint, sabotage and news-planting for free — which does raise a non-shielded city's ambient planted-report chance from ~23% to ~32%, the real consequence of a new rival and the same one Chapter 2 accepted for Doria. Adorne's house is still unbuilt: he appears in Chapter 5 only through his niece, as a family connection rather than a rival company.
...
>
> **Phase 19 note, confirmed 2026-07-30:** screen 7 (Evidence board) is built — `components/EvidenceBoardPanel.tsx` plus `sim/dossier.ts`, reached from its own "Dossier" rail section, which also carries the divining gift since both belong to §8's personal plot layer rather than to the commercial game (the Household screen is for officers who draw a salary, and Nicholas isn't one). Both of §8's tracks are there: the parentage dossier, shown deliberately without any progress indicator because its answer is Chapter 8's, and the Vatachino unmasking, which does have a threshold and shows progress toward it. "Chapter secrets" stay where they already are, in the existing Secrets panel — evidence and secrets are modelled as different things on purpose (a secret has a florin value and is spent by selling; evidence is never sold and only means anything in combination), so folding them into one screen would have meant either a sellable dossier or a zero-value secret the Secrets panel would then need to learn to hide. The board is read-only, the same discipline `ObjectivesPanel` uses. The rail tab only appears once the board has something on it — the same conditional inclusion the Estate tab already uses, and for the same reason: an always-present tab reading "nothing pinned here yet" for four chapters is worse than no tab.
>
> **Phase 12 note, confirmed 2026-07-23:** the interactive guided tour (`components/GuidedTour.tsx`, not itself a design-doc screen — a coach-mark overlay added post-Phase-11) grew from covering only the Map/Market trade loop to also spotlighting Household (assignments), Dispatches (courier investment), and the Ledger (writing a bill), and is now gated to run after Chapter 0 completes rather than at week 0, since Chapter 0 owns the player's literal first moves now. Insurance, Houses, and Secrets remain mentioned only narratively in its closing card, not forced through — the same "don't force a screen's every feature" restraint Phase 11's own note above already used for the Venture planner.
